...
def test_chunked_prefill_compacts_only_on_first_decode_and_reuses_shared_kv(
    ...
    monkeypatch.setattr(hf_gemma4, "_block_state", lambda block: ())
...
def test_moe_sliding_layer_uses_the_compact_cache_index(monkeypatch):
    ...
    # A full-length cache here would still be numerically correct, but the
    # custom op would scan all of it on every decode step. Diagnose a broken
    # allocator/compaction handoff instead of silently losing SWA performance.
    full_keys = [torch.zeros(1, 1, capacity + 64, 8)]
    full_values = [torch.zeros_like(full_keys[0])]
    with pytest.raises(RuntimeError, match="expected compact capacity"):
        _run_blocks_over_embeds(
            model,
            torch.zeros(1, 1, 8),
            torch.tensor([[prompt_len + 1]]),
            torch.zeros(1, 1, 1, capacity + 64),
            full_keys,
            full_values,
            make_cache_index(prompt_len + 1, 1),
        )
