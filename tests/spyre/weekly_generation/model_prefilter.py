...
One check, applied where ``weekly_test.main`` used to apply it in-process.
Models that are too large for Spyre produce a row recording that verdict, so a
dropped model is never silently absent from a run's output.
...
can cluster by download count, so a run of oversized models can hollow out one
shard while leaving the next untouched. Filtering after chunking left shards
with wildly different amounts of real work:
...
def fetch_and_filter(
    ...
    # filter models that exceed the Spyre parameter limit
