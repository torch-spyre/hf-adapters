"""ClickHouse schema and client factory for the model_spyre_support tables.

Imported by ``result_sink``; not runnable on its own. The DDL below is the single
source of truth for the table shape — ``TABLE_COLUMNS`` must stay in the same
order as the ``CREATE TABLE`` body and as the values ``ResultSink`` subclasses
write.
