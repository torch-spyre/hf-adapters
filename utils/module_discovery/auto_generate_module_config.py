...
# Modules that upstream's own module_db already covers are NOT filtered out.
#
# This used to drop any module whose class name appeared in
# ``torch.testing._internal.common_modules.module_db`` -- which silently removed
# every ``nn.Linear`` and ``nn.Embedding`` entry, i.e. the projections and the
# token embedding, from the generated config. Two things were wrong with it:
#
# 1. **It discarded the only information worth capturing.** Upstream's module_db
#    exercises ``Linear(10, 8)`` and ``Linear(3, 5)`` on default layouts. A
#    capture records ``[1, 128, 12800]`` with the pinned Spyre ``device_layout``
#    the adapter actually runs. Sharing a class name does not make those the same
#    test; the captured shape/layout is the entire point of this generator.
#
# 2. **It was environment-dependent, so it silently changed the output.**
#    Importing module_db needs ``expecttest``, which is a dependency of the
#    oot_framework extra and absent otherwise. So the same command emitted
#    different module sets depending on whether that extra happened to be
#    installed: with it, 114 names were filtered (Linear, Embedding, SiLU gone);
#    without it, the ImportError branch filtered nothing. The YAMLs in
#    tests/configs/module_tests were generated by the unfiltered path, so
#    regenerating them in an oot_framework env quietly dropped modules.
#
# Duplicate *names* within one config are already prevented by the per-instance
# naming (``Linear_layer0_prefill_h12800``), so nothing here needs a class-level
...
# The subset of extracted config attributes that describe tensor SHAPE rather
# than arithmetic. Used by ``_extract_config_kwargs`` to decide whether a None
# value is worth recording: a missing dimension should be visible in the YAML,
# whereas a None multiplier/softcapping just means "use the default".
_SHAPE_CONFIG_ATTRS = frozenset(
    {
        "hidden_size",
        "num_attention_heads",
        "num_key_value_heads",
...
def _extract_config_kwargs(
    ...
    for attr in [
        # --- shape: what the module's tensors look like ---
        "hidden_size",
        "num_attention_heads",
        "num_key_value_heads",
        "intermediate_size",
        "max_position_embeddings",
        # --- numerics: scalars baked into the module's arithmetic (see below) ---
        # These carry no shape, so omitting them still produces a module that
        # builds and runs -- with different MATH than production, which is worse
        # than a crash because the test passes while comparing the wrong thing.
        # Granite reads `attention_multiplier` straight into `attn.scaling`; when
        # it is missing the config default (1.0) applies instead of the
        # checkpoint's value (0.0078125 for Granite 3.3 8B).
        #
        # `scaling == 1.0` is also a distinct COMPILE hazard, not only a numeric
        # one: SDPA lowers to `(q @ k^T) * scale`, and at exactly 1.0 that
        # multiply is an identity that gets folded away. The non-contiguous RoPE
        # output then flows straight into the matmul's restickify instead of being
        # materialized by the multiply, producing::
        #
        #     insert_restickify_padding: sliced output on bufNN
        #     (written size 64 < device dim size 128) cannot be padded in place
        #
        # Measured: every other scale (0.5, 2.0, 1/sqrt(128), Granite's
        # 0.0078125) compiles; only 1.0 fails. So a config missing
        # `attention_multiplier` made the module test hit a compile failure the
        # production path never sees.
        "attention_multiplier",
        "residual_multiplier",
        "embedding_multiplier",
        "logits_scaling",
        # Gemma's equivalents: `query_pre_attn_scalar` sets the SDPA scale the
        # same way `attention_multiplier` does, and the softcapping values change
        # the attention/logit arithmetic.
        "query_pre_attn_scalar",
        "attn_logit_softcapping",
        "final_logit_softcapping",
    ]:
        value = _read_config_attr(config, attr)
        if value is _CONFIG_ATTR_MISSING:
            continue
        # A numerics attribute present but None means "disabled", which is already
        # the config class's default (Gemma's two softcapping fields are None on
        # every checkpoint that does not use them). Emitting `attr: null` would
        # add noise to every generated YAML, and a null reaching the constructor
        # is not always equivalent to omitting it, so drop it and let the default
        # apply. Shape attributes are kept even if None -- a None dimension is a
        # signal worth seeing in the YAML, not a default to fall back to.
        if value is None and attr not in _SHAPE_CONFIG_ATTRS:
            continue
...
def _extract_device_layout(tensor: torch.Tensor) -> Optional[Dict[str, Any]]:
    """Read a Spyre tensor's device layout, or ``None`` if it has none.

    A tensor captured from a model already moved to Spyre (``--loader spyre``)
    carries a ``SpyreTensorLayout``: the device-side shape and strides, which are
    decoupled from the logical shape (see ``hf_common._cache_position_first_stl``).
    Recording it lets the generated YAML rebuild the input with the SAME layout
    the production path used, instead of whatever default ``.to("spyre")`` picks.

    Returns ``None`` -- so the caller omits the key entirely -- for a CPU tensor,
    when torch-spyre is unavailable (off-pod dry run), or when the query fails.
    A missing layout is the normal case for the HF-loader path and must not break
    generation, so every failure mode degrades to "no device_layout" rather than
    raising.

    ``device_dtype`` is emitted as a torch dtype alias (e.g. ``torch.bfloat16``),
    which is what ``SpyreTensorLayoutSpec`` in the OOT framework expects; it
    resolves it back through ``get_device_dtype()``.
    """
    if tensor.device.type != SPYRE_DEVICE:
        return None
    ...
    except Exception:  # noqa: BLE001 - torch-spyre absent, or tensor has no layout
        return None
    ...
    try:
        ...
        stride_map = [int(s) for s in layout.stride_map]
    except Exception:  # noqa: BLE001 - unexpected layout object shape
        return None

    if not device_size or len(device_size) != len(stride_map):
        # The OOT framework's SpyreTensorLayoutSpec validator rejects a length
        # mismatch, so emitting one would produce a YAML that cannot be loaded.
        return None

    return {
        "device_size": device_size,
        "stride_map": stride_map,
        # The layout's own device_dtype is a DataFormats enum, which does not
        # round-trip through YAML. The framework re-derives it from a torch dtype
        # alias, and asserts it equals get_device_dtype(tensor.dtype) -- so the
        # tensor's dtype is the only value that can pass that check.
        "device_dtype": str(tensor.dtype),
    }
...
def _extract_tensor_info(tensor: torch.Tensor, name: str) -> Dict[str, Any]:
    ...
    device_layout = _extract_device_layout(tensor)
    if device_layout is not None:
        info["device_layout"] = device_layout
    return info
...
class ModuleInfoCapture:
    ...
    def capture_constructor_info(
        ...
        # Check if module has a config attribute (common in Transformers)
            ...
            # nn.Linear(in_features, out_features, bias=...). Both are required
            # positional args, so without them the framework's
            # ``module_cls(*args, **kwargs)`` raises::
            #
            #     TypeError: Linear.__init__() missing 2 required positional
            #     arguments: 'in_features' and 'out_features'
            #
            # Read them off the module rather than ``weight.shape`` so a Linear
            # subclass that stores its weight transposed still reports the sizes
            # its own constructor expects.
            #
            # ``bias`` must be recorded too: it defaults to True, but the
            # attention/MLP projections these captures cover are built with
            # ``bias=False``. Rebuilding them with a bias changes both the
            # parameter set and the arithmetic.
            constructor_args.append({"type": "int", "value": module.in_features})
            constructor_args.append({"type": "int", "value": module.out_features})
            constructor_kwargs["bias"] = {
                "type": "bool",
                "value": module.bias is not None,
            }
        elif isinstance(module, nn.Embedding):
            # nn.Embedding(num_embeddings, embedding_dim, padding_idx=...).
            # Same missing-positional-args failure as nn.Linear above.
            #
            # ``padding_idx`` is recorded when set: it zeroes that row and excludes
            # it from gradients, so omitting it rebuilds a different module.
            constructor_args.append({"type": "int", "value": module.num_embeddings})
            constructor_args.append({"type": "int", "value": module.embedding_dim})
            if module.padding_idx is not None:
                constructor_kwargs["padding_idx"] = {
                    "type": "int",
                    "value": module.padding_idx,
                }
        else:
    ...
    def _create_unique_module_name(
        ...
        Examples:
            ...
            Linear(4096, 12800) inside layer 0 -> Linear_4096_12800_layer0
        """
        ...
        # Several int args: every one of them goes in the name. ``module_data`` is
        # keyed by this NAME, not by ``config_signature``, so two variants that
        # differ only in their constructor dimensions must not produce the same
        # name -- the first one captured would own the entry and the rest would be
        # recorded against its constructor args.
        #
        # That is not hypothetical: one decoder layer holds seven ``nn.Linear``
        # instances (q/k/v/o plus the MLP's gate/up/down) at three distinct
        # shapes, and "the layer index alone disambiguates" below is only true for
        # classes appearing once per layer. Collapsing them emitted
        # ``Linear_layer0_prefill_h12800`` -- an entry fed a ``[1, 128, 12800]``
        # input while carrying ``Linear(4096, 4096)`` constructor args.
        if len(args) > 1 and all(a["type"] == "int" for a in args):
            return _compose_module_name(
                module_type, *(str(a["value"]) for a in args), layer_suffix
            )
    ...
    def _create_invocation_signature(
        ...
        def _extract_pattern(input_info: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if "type" in input_info and "items" in input_info:
                    ...
                    "items": [
                        {
                            ...
                            "init": item.get("init"),
                            "device_layout": item.get("device_layout"),
                        }
                ...
                # Single tensor. The device layout is part of the pattern: two
                # invocations can share a shape/dtype yet be handed tensors laid
                # out differently on device, and collapsing those would drop a
                # layout the module is really called with.
                return {
                    "type": "tensor",
                    "shape": input_info.get("shape"),
                    "dtype": str(input_info.get("dtype")),
                    "init": input_info.get("init"),
                    "device_layout": input_info.get("device_layout"),
                }
            else:
                # Unknown structure
                return {"type": "unknown"}
...
def get_all_custom_modules(
    ...
    for name, module in model.named_modules():
        ...
        # Intentionally NOT filtered against upstream's module_db -- see the note
        # on ``existing_modules``: a shared class name does not mean a shared
        # test, and the captured shape/layout is what this generator exists for.
...
def _tensor_info_to_spec(tensor_info: Dict[str, Any], name: str) -> Dict[str, Any]:
    ...
    # A tensor captured on Spyre carries the device layout the production path
    # gave it. Emitting it lets the test rebuild the input with that same layout
    # rather than the default one ``.to("spyre")`` would choose. Absent for
    # CPU-captured tensors, which have no layout to record.
    device_layout = tensor_info.get("device_layout")
    if device_layout is not None:
        tensor_spec["device_layout"] = device_layout
...
def _build_module_entry_dict(module_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # int/bool/float/str all pass through by value. This used to accept "int"
    # only, which silently dropped every other kwarg -- e.g. nn.Linear's
    # ``bias=False``, which would then be rebuilt with the default bias=True.
    for key, kwarg_spec in module_info.get("constructor_kwargs", {}).items():
        if kwarg_spec["type"] in ("int", "bool", "float", "str"):
