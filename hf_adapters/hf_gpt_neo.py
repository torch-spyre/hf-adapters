...
def _make_compiled_block(layer):
    ...
    ``attn.head_dim`` may have been updated by ``pad_attention_heads_linear`` to a padded
    ...
    head_dim = (
        attn.head_dim
    )  # padded if pad_attention_heads_linear ran, original otherwise
...
def prepare_for_spyre(model):
    ...
    if padded_head_dim > orig_head_dim:
        pad_attention_heads_linear(
            model,
            (layer.attn.attention for layer in bb.h),
            orig_head_dim,
            padded_head_dim,
            num_heads,
        )
