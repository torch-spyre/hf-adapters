...
tests: ## Run the suites selected by TEST_TYPE into RESULTS_DIR (JUnit per suite)
	...
	for suite in $$suites; do \
	  ...
	  case "$$suite" in \
	    ...
	    token_classification_compare) mkdir -p "$(RESULTS_DIR)/junit-token-classification-compare" && $(MAKE) token-classification-compare-tests JUNIT_XML="$(RESULTS_DIR)/junit-token-classification-compare/junit-token-classification-compare.xml" MODEL_KEY="$(MODEL_KEY)" || rc=1 ;; \
